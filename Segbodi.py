...
def segment_body(original_img, face=True):
    ...
    if isinstance(original_img, str):
        model_image1 = Image.open(original_img)
    else:
        model_image1 = original_img 
    # Segment image
    ...
    segment_include =[ "Upper-clothes","Skirt", "Right-arm", "Left-arm"]
    mask_list = []
    for s in segments:
    ...
    # Remove face
    if(face==False):
        final_mask = remove_face(model_image1.convert('RGB'), final_mask)

    # Apply mask to original image
    model_image1.putalpha(final_mask)

    return  final_mask,model_image1
